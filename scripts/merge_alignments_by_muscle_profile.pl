...
foreach my $filename (@filenames) {
    ...
    if ($idfilename{$id}) {
        ...
        if ($region1 eq $region2) {
            ...
            my $outfile = $id."_".$firstTP."-".$secondTP."_".$region1."_NT_1stTPUncollapsed.fasta";
            system("muscle -profile -quiet -in1 $firstfile -in2 $secondfile -out $outfile");
        }else {
            die "region is different: $region1 vs. $region2\n";
        }      
    }else {
        die "No corresponding second alignment for $filename\n";
    }
